...
var $al_JQuery = jQuery.noConflict();
...
function ajaxlike_fulllistbox(post_id, topic_id, forum_id, callback_url, like_on_text)
{
	...
	$al_JQuery(function() {
		...
		$al_JQuery( "#dialog:ui-dialog" ).dialog("destroy");
		
		$al_JQuery( "#ajaxlike-dialog" ).dialog({
			width: '500',
			height: '400',
			modal: true,
			position: 'center',
			...
			buttons: {
				OK: function() {
					$al_JQuery( this ).dialog( "close" );
				}
			},
 			beforeClose: function(event, ui) { 
  				$al_JQuery("body").css({ overflow: 'inherit' }); 
			}, 
			open: function(event, ui) {
  				$al_JQuery("body").css({ overflow: 'hidden' });
				...
				$al_JQuery.getJSON(callback_url, {
					ajaxlike_rnd : rv,
...
function ajaxlike_notificationsbox(callback_url)
{
			...
			if(data!=null)
			{
				...
				for(i=0;i<data.length;i++){
					...
					$al_JQuery("#ajaxlike_not_new").html(""+"<strong>" + nlikes + "</strong>" + ninfo +  "");
	  				$al_JQuery("#ajaxlike_not-dialog").fadeIn("slow");	
	   				$al_JQuery('div').on('hover',function() {  
         			hoveredId = $al_JQuery(this).attr('like_id');
         			var like_id =(hoveredId);
		 			$al_JQuery(".close" + like_id).click(function () {
        			//close notification when the close button is clicked
        			$al_JQuery("#box" + like_id).fadeOut();
          			});
		  			
		  			}); 
		  		  	
       				setTimeout(function(){
      				$al_JQuery("#ajaxlike_not-dialog" ).fadeOut(3000);
	  	 			},5000); 
...
{   
	...
	$al_JQuery(function() {
		...
		$al_JQuery( "#dialog:ui-dialog" ).dialog("destroy");
			...
			$al_JQuery( "#ajaxlike-not-dialog" ).dialog({
			width: '500',
			height: '400',
			modal: true,
			show: "fade",
			...
			var rv = new Date().getTime();
				...
				$al_JQuery("#ajaxlike_not_new").html(""+"<strong>0</strong>" + ninfo +  "");
